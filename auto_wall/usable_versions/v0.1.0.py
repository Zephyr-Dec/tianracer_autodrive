...
class SideWallFollower(Object):
    def __init__(self):
        ...
        self.balance_factor = 0.6 # bilateral balance factor(0-1)
    ...
    def calculate_wall_error(self, dis, a):
        ...
        AB = dis * np.cos(alpha)
        return 1.0 - (AB + 3.0 * np.sin(alpha)) # deviation=target(1m)-projection distance
    ...
    def scan_callback(self, data):
        try:
            ...
            right_a = self.get_range(data, -30) # the value at thirty degrees
            ...
            # dynamic speed ctrl
            base_speed = self.min_speed + (front_dis / self.safe_distance) * (self.max_speed - self.min_speed)
            if track_condition == "straight":
                base_speed *= self.speed_boost

            elif track_condition == "right_corner":
                base_speed *= 0.9
            else:  # left turn
                base_speed *= 0.85
            ...
            # release ctrl commands
            drive_msg = AckermannDriveStamped()
            drive_msg.drive.steering_angle = np.clip(steering_angle * min(1.0, self.track_width / 5), -0.5, 0.5)
